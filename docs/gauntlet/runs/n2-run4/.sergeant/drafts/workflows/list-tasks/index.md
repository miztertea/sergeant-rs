...
description: >-
  ...
  Completion: The sole inferred stage `list-tasks` has reached its outcome (design inference — see `provenance.md`).
tags:
  - draft
  - repo-to-icm-n2-run4
---
...
No `stage`-rung record in this run's corpus carries `workflow=list-tasks` directly; this candidate's evidence is its workflow-level helpers alone (see `provenance.md`).
