...
- **Vertical, not horizontal.** Horizontal slicing (writing all tests
  ...
  tracer bullet responding to what the last cycle taught. — `BU-P2-113`
...
- **J0 the caller must honor:** seam confirmation is not a J2 judgment
  ...
  unconfirmed seam (`BU-P2-109`). A stage applying this technique must
...
- The workflow-level trigger and purpose statement (`BU-P2-104`,
  `BU-P2-105`) are subsumed by this file's own opening paragraph; they
  were never independent of the technique content itself.
- Refactoring was already explicitly out of scope for the loop
  (`BU-P2-116`) before this rehome — that boundary is unchanged, only its
  representation is.
