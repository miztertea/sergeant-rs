...
- Whether to write a test at an unconfirmed seam — J0, inherited from the
  TDD discipline this workflow applies in `10-implement-with-tdd` (see that
  stage's own Bounded judgment section; the underlying `tdd` package's own
  placement is presently under independent ICM-R3 dispute — see that
  stage's tracking note — but the seam-confirmation requirement itself is
  confirmed regardless of how that dispute resolves).
...
- `10-implement-with-tdd` applies the `tdd` discipline. As of this
  revision the current mechanism for doing so is unsettled: `tdd`'s own
  ICM-R3 placement is disputed between REHOME (a `.sergeant/common/
  contexts/tdd.md` shared context) and its independent reviewer's
  objection that the alternatives analysis never weighed a PL-7 engine-gap
  representation. See that stage's own `CONTEXT.md` for the current
  tracking note; this section will be revised once `tdd`'s dispute
  resolves.
