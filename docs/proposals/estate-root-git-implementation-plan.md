...
|-------|-----------------|----------|-------------------|
| 0 | Slice 0 + C11 | Failing contract-pin tests (exact-root, zero-config removal, scope refusal, teardown branch mismatch, common-dir lock aliasing); injectable git binary; baseline doctor/branch totals recorded (pre-sweep bundles of 2026-08-20 preserve the #172/#159 evidence — landed as `estate-root-git-baseline-2026-08-20.md`, which records why the inventory rather than a `.bundle` is the artifact) | test scaffolding for everything after |
