...
| `source.quote_hash` | yes | `sha256:<hex>` of the exact quoted source text the statement was normalized from. **A behavior unit whose `source` is present but whose `quote_hash` is missing, empty, or does not verify against the cited locator is invention, and is rejected at lint, not review** (N1 contract, binding rule). This is the strongest rule in this document — every other field has judgment-call cases; this one does not. |
...
| `representation` | yes | One of the ladder's terminal representations: `invariant` (§6.1, → `AGENTS.md`), `workflow` (§6.2), `stage` (§6.3), `actor-stage` (§6.4, → stage with `CONTEXT.md`), `helper` (§6.5), `shared-helper` / `shared-context` (§6.6), or `engine-gap` (§6.7). No other value is legal; a representation outside this set means the ladder was not actually applied. |
...
| `alternatives_considered` | yes | The other representations weighed and rejected, as a list of ladder rung names. An empty list is a violation whenever more than one rung was plausible on its face (which the ladder's ordered-question structure makes true for almost every unit — see §6's "ask these questions in order"); an empty list is only legal when the unit trivially fails every rung but one (e.g. a behavior that is obviously and only a helper, with no plausible reading as a stage). |
