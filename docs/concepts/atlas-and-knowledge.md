...
Repository sources are read differently and never through the working tree. A repository is indexed at the commit its admission pinned, straight out of the Git object store, in batched object reads — no fetch, no pull, no branch switch, nothing written. If the mount's `HEAD` moves while a scan is running, the scan stays on the commit it pinned and the move is reported beside the result as a drift observation rather than blended into it. A Work surface *can be* indexed as an *overlay* on its base commit — files the Work changed hashed from the surface, everything else keeping the base tree's own object ids, scoped to that Work and removed when the Work is retired — and the code that does this is built and tested. **It has no trigger yet.** Nothing in this build ever calls it outside a test: no route, no CLI verb, and no Work-lifecycle hook invokes it on admission or retirement, unlike the three source kinds `sgt intelligence scan` actually drives (below). A real Work's overlay evidence cannot be produced on a running installation today. Wiring one is a later wave's, when a consumer needs it.
...
| D14 | Work-surface overlay indexing is built and correct but has no trigger: nothing in this build's routes, CLI, or Work lifecycle calls it. `sgt intelligence scan` drives all three source kinds; it does not drive an overlay. |
