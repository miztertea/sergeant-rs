...
Containers — a ZIP archive, a saved `.eml` message — are not knowledge formats, so they are opened and their members become resources in their own right. An admitted entry or attachment is addressed at its container's path plus `!/` plus its own entry path (`bundle.zip!/notes/a.md`), carries its own content hash and the parent coordinate it came from, and is extracted by whatever adapter its own name routes to — a `.docx` inside a `.zip` inside an `.eml` is read by the Office adapter, exactly as a loose `.docx` is. A member nothing in this build claims is reported `unsupported` at its own path rather than passing unmentioned. Expansion is bounded: unsafe entry paths and credential-shaped names are refused before any byte is kept, every member is subject to the same per-resource size ceiling a file on disk is, and nesting is capped, so a member past the depth cap is recorded as a resource and simply never opened.

One limit stated plainly rather than implied: the parser that opens a container runs in a supervised worker process, not in the daemon, so the daemon hashes a member's bytes when it *receives* them. A member's content hash therefore identifies the bytes that reached the store — it is not a claim about what is inside the operator's archive, which the daemon never opened itself.
...
A scan of a real estate takes minutes, so the verb does not wait on one request for it. The daemon accepts the scan, names it, and runs it; the client follows that named scan and prints each source's row as that source finishes — `[2/5]`, then `[3/5]` — and exits zero when the scan itself reports completion. Both ends of the run are journaled (`intelligence.scan.started` names every source the scan undertook; `intelligence.scan.completed` carries the per-source outcome tally and how long it took), so what the daemon did is in the journal whether or not any client was watching, and a scan that died mid-flight is legible as attempted-and-unfinished rather than absent. `sgt intelligence status` remains a separate question — what this host has indexed — and is not where completion is read from.


## The search surface

`sgt search <query>` is one query across everything the estate can see, and `sgt related <coordinate>` finds units related to one already-retrieved coordinate. Both are **pure readers**: asking a question never scans, indexes, refreshes or warms anything. `sgt intelligence scan` is what changes what is searchable.

Ranking is deterministic and stated. The world is filtered first — by source, by Work, by content family, by source kind — and only then ranked: a BM25 lexical list and, where a semantic model is installed, an exact-cosine list, fused by Reciprocal Rank Fusion and reranked on structural facts the index already holds. There is no weight to tune, on the CLI or in a workflow file; the constants are constants.

Selectors narrow the world before ranking: `--source <name>` (or `<name>@<content-key>` to pin one exact generation), `--work <id>` (with `--repo` when the Work targets more than one repository), `--content code|document|email|row-text|all`, `--type repo|knowledge|external`, `--external`, `--top <n>`, and `--no-semantic`. `--content config` is named in the retrieval contract and is **refused with its reason** in this build: nothing stored distinguishes a config file read through the text fallback from an ordinary text document, so a `config` lane would return some config files as though it had returned all of them.

Every hit cites its source, the exact generation, the unit, and a coordinate shaped for its family — a code hit carries language, label, symbol and a byte span; a document or mail hit carries a title and a byte span; a structured-row hit carries a dataset key, a row key and the exposed field set, and no byte span at all, because a row is read in place and never copied. Every hit also carries its source's kind and authority class, so evidence acquired from outside the estate stays visibly external in the answer itself rather than only in the store.

Every answer states whether the semantic half participated — `applied`, `not_installed` or `disabled` — as a value that is always present, so a degraded answer cannot be mistaken for a complete one. `--json` additionally carries the full search trace: the query and its hash, the Work attribution when there is one, the filters applied, the exact source generations searched, the tokenizer and its version, the semantic model identity and hash when one was used, the ranking policy version, and every result's evidence id and rank.

The coordinate `sgt search` prints is the coordinate `sgt related` takes. `related` ranks neighbours over the anchor unit's own indexed text, inside the same filter, and never returns the anchor as its own neighbour; an anchor the filter excludes resolves to nothing rather than to its neighbours.
...
| D15 | `sgt search` and `sgt related` are pure readers with no write path of any kind — no index-on-demand, no scan-if-stale, no cache warming. Every answer carries the deterministic filter it was computed under, whether the semantic half participated, and (in `--json`) the full search trace. No retrieval weight is exposed on the CLI or in a workflow file. |
