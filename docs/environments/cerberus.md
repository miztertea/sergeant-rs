...
| Live user daemon | an installed `sgt` runs on this host outside the repo: `~/.cargo/bin/sgt --data-dir ~/.local/share/sergeant daemon` (pid 35846 at first contact). **Never kill it, never point tests at its data dir** — test leak checks pattern on `debug/sgt`/`release/sgt` paths and cannot collide with it | `pgrep -af sgt`, 2026-08-11 |
