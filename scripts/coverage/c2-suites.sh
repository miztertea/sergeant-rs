...
# S6 P0, wired at birth (the #231 lesson, same as a2/x5 above): the C1 §21
# acceptance register — the fourteen-item walk, its citation guard and its
# doc-table guard. Reads this repository's own test sources only — no daemon,
# no estate, no subprocess. Floor 1.
cov_stage_begin c2-c1_acceptance
cov_run cargo llvm-cov --no-report --test c1_acceptance --locked || cov_fail "c1_acceptance failed under instrumentation"
cov_stage_end 1 "the c1_acceptance test binary must write its own profile"
...
# S6, wired at birth (the #231 lesson, same as w1/w1b/w1c/w1d/w2/w3/w3b
# above): the semantic half answers from vectors persisted at SCAN time
# rather than re-embedding the corpus on every query, and an index that
# carries no vectors for the loaded model says `not_indexed` instead of
# claiming a ranking it cannot produce (A1 §15). Loads the committed model
# once per test process, like w3b_semantic_retrieval; builds Atlas stores in
