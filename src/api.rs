...
pub const SSE_EVENT_KINDS: &[&str] = &[
    ...
    // S6 scan front door: the two scan-level kinds, one each per accepted
    // scan. They join the vocabulary through this same shared list (C1d's
    // route) so a client enumerating frame names cannot silently drop the
    // only frames that say a scan started and finished.
    crate::domain::source::KIND_INTELLIGENCE_SCAN_STARTED,
    crate::domain::source::KIND_INTELLIGENCE_SCAN_COMPLETED,
];
