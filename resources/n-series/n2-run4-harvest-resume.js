...
${firstAttempt ? `SEEDING (first attempt only, run-4 specific, recorded in your notes): before anything else, verify output/ is empty of prior artifacts, then copy run 3's committed harvest outputs from ${SEED}/ (README.md, behavior-units.ndjson, consequence-class-sweep.md, partition-ledger.md) into your stage's output/. These are the same subject SHA (verify against reference/UPSTREAM.md's pin) and the same v2 workflow content — run 3's ledger has P1–P6 done (BU-0001–BU-0312). Then verify the ledger's 21 partition names match ../10-inventory/output/inventory.md from THIS run exactly (order and names). Any mismatch: STOP, do not harvest, report it in notes with both lists — that is a fail-closed condition the orchestrator must see.` : 'The ledger already exists in output/ from prior attempts — the protocol\'s retry rules apply: trust done rows, work the next pending partitions in order.'}
