...
If output/ already contains the seeded run-3 artifacts and the ledger's scheme-provenance section (a prior attempt already ran), do NOT re-seed and do NOT treat the non-empty output/ as a violation — behave as a continuation attempt: trust the ledger's done rows (the units file's highest BU id matches the highest done range), apply the ruling above if its subsection is missing, and continue at the next pending partition.
MECHANICS (measured, docs/gauntlet/notes/n2-fake-backend-semantics.md): the stage is held on needs_input and the hold PERSISTS across actor attempts — you are an external actor; no retry, no fresh execution, no engine command is needed for you to work. Only the final attempt (ledger all done) sends the ONE respond that advances the stage.
...
THEN: if EVERY ledger row is done, advance the stage: ${setup.respond_cmd.replace('WORK_ID', setup.work_id).replace('TEXT', '"stage 20-harvest complete"')} and set advanced=true. If pending rows remain, do NOT respond and do NOT issue any engine command — just end, reporting honestly (the next fresh actor continues against the same held stage).`,
  ...
  // No engine command between attempts: the needs_input hold persists and
  // actors are external (measured semantics, n2-fake-backend-semantics.md).
  // The next loop iteration's fresh actor continues against the same hold.
}
