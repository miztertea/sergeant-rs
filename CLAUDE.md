...
- Tests live in per-milestone suites `tests/m1_event_core.rs` … `tests/m8_estate_cli.rs` (529 total + 4 opt-in as of the MVP-3 fixer pass, 2026-08-12 — coverage 94.63% lines measured at S2 close by the committed convention, `docs/coverage/baseline-2026-08-10.md`; the count is a smoke check on this line, not a budget, and TH-R2-03 confirmed round 1's TH-9 fix here had already gone stale again by the very next diff — re-measure with `cargo test` rather than trusting this line). Suites that spawn daemons MUST go through `tests/support/mod.rs`'s `DataDir` guard — the `sgt(...)` helpers take `&DataDir` so an unreaped auto-spawned daemon is a type error, and the guard reaps by `/proc` argv scan on Drop. This exists because a measured leak accumulated ~89 orphan daemons in a day.
