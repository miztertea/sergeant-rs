...
- Tests live in per-milestone suites `tests/m1_event_core.rs` … `tests/m6_surfaces.rs` (269 total + 3 opt-in as of N3 wave 2; the count is a smoke check on this line, not a budget). Suites that spawn daemons MUST go through `tests/support/mod.rs`'s `DataDir` guard — the `sgt(...)` helpers take `&DataDir` so an unreaped auto-spawned daemon is a type error, and the guard reaps by `/proc` argv scan on Drop. This exists because a measured leak accumulated ~89 orphan daemons in a day.
