...
- Tests live in per-milestone suites `tests/m1_event_core.rs` … `tests/m6_surfaces.rs` (352 total + 3 opt-in as of the N3/S2 merge — coverage 94.63% lines measured at S2 close by the committed convention, `docs/coverage/baseline-2026-08-10.md`; the count is a smoke check on this line, not a budget). Suites that spawn daemons MUST go through `tests/support/mod.rs`'s `DataDir` guard — the `sgt(...)` helpers take `&DataDir` so an unreaped auto-spawned daemon is a type error, and the guard reaps by `/proc` argv scan on Drop. This exists because a measured leak accumulated ~89 orphan daemons in a day.
