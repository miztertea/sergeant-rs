...
- **While the daemon runs, no further bulk-reconciliation pass exists or is needed: the daemon observes every in-flight execution in-process — at launch-settle, at SEND-settle, and on client cranks (the four mutating handlers — submit, respond/SEND, retry, cancel; a read-only request answers from the projection instead) — and periodically between those instants via a completion-driver sweep (`due_observations`/`due_interrupts`, `src/api.rs`'s `drive_completions`) that exists specifically because a real turn can finish at none of the other instants — so fleet state does not go stale between one restart and the next. There is deliberately no on-demand sync verb: the upstream `--sync-all` was file-drift repair for a daemon-less shell tool with no continuous supervisor of its own; this daemon has one.**
