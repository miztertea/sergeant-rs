...
- Whether a ticket is HITL or AFK is settled by the ticket's own type label for the two types this package's stage prose names (research is AFK, task is either).

### J2 — delegated to this stage
- Which ticket to choose when the user has not named one.
- Whether an in-progress ticket turns out to be out of scope during resolution.

### J1 — local choices allowed
- None beyond ordinary tool mechanics.

### J0 — must become `needs_input`
- A HITL ticket's own question: the agent never answers on the human's behalf.
