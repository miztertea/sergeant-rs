...
- Which construction strategy to attempt first from the ranked ladder, and how to tighten it along the three named axes.

### J1 — local choices allowed
- Local tooling/script naming within the chosen construction strategy.

### J0 — must become `needs_input`
- **No loop can genuinely be built.** Stop, list what was tried, and ask the user for environment access, a captured artifact, or permission to add temporary production instrumentation — never proceed to hypothesize without a loop.

### Completion boundary
This stage may complete only when the actor can name one already-run command that is red-capable, deterministic (or pinned high-rate for flaky bugs), fast, and agent-runnable.
