...
**Resolved (was BU-VAS-15's push/pr/ci placeholder, issue #123):** the run this stage drives never reaches a push/PR/CI step in the first place — `30-start-run`'s own `axi run` invocation always includes `--skip push,pr,ci` (its own J5 fixed rule), because this workflow, shipped as this repository's own tool for developing itself, encodes this repository's own development practice (`docs/DEVELOPMENT.md`: publication stays human-owned). This is workflow content, not a product-wide engine restriction — Sergeant executes work, and a user's own workflow for their own repo is free to omit that flag. Measured live twice before this fix (`docs/gauntlet/runs/path-to-mac-2026-08-15/retrospective.md` §3.1; `docs/gauntlet/runs/macbook-arrival-2026-08-15/retrospective.md` §3); nothing for this stage to authorize now.
