...
- Distinguishing `checks-passed`/`passed`/`failed`/`cancelled` and taking the matching path — stop and hand off, fix and re-drive, or explain the blocker.
- Summarizing what the pipeline found and fixed for the user.

### J1 — local choices allowed
- Summary wording, provided every pipeline-made fix is explicitly acknowledged.
