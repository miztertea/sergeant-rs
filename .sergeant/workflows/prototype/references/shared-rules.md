...
model. Carries forward `BU-P3-015`, `BU-P3-016`, `BU-P3-017`, `BU-P3-018`
from the N1 harvest (`reference-corpus/behavior-units/P3.ndjson`), each
already classified `representation: shared-context` at N1 but never
actually materialized anywhere in this package before ICM-R3 (finding
`BU-PROTO-28`, corrected per the independent reviewer from an initial
"harvest gap" misclassification — these four were already extracted,
just never built; only a fifth item, "skip the polish," remains a
genuine unbuilt harvest gap, tracked separately, not drafted here).
...
- **Throwaway from day one, and clearly marked as such.** Locate the
  ...
  — `BU-P3-015`, `reference/sergeant-upstream/.agents/skills/prototype/SKILL.md` (item 1, line 21)
- **One command to run.** Whatever the project's existing task runner
  ...
  — `BU-P3-016`, `reference/sergeant-upstream/.agents/skills/prototype/SKILL.md` (item 2, line 22)
- **No persistence by default.** State lives in memory. Persistence is
  ...
  — `BU-P3-017`, `reference/sergeant-upstream/.agents/skills/prototype/SKILL.md` (item 3, line 23)
- **Surface the state.** After every action (logic) or on every variant
  ...
  see what changed.
  — `BU-P3-018`, `reference/sergeant-upstream/.agents/skills/prototype/SKILL.md` (item 5, line 25)
...
(`BU-PROTO-28`'s review), not fabricated here.
