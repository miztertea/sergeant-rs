...
| `rationale` | yes | States *why this rung and not an adjacent one*. A rationale that would read identically pasted onto a different representation choice does not discriminate and is a violation — see icm-ladder.md's reimplementation test for `stage` records specifically. **For `stage-context`, `helper`, `shared-helper`, or `shared-context`, `rationale` MUST state the §6.3 reimplementation-test answer explicitly (as its own sentence, before the rung-specific reasoning) even though the answer was "no" — a rationale that jumps straight to "this is deterministic machinery" without that sentence has skipped §6.3, not cleared it (`../_config/icm-ladder.md`, N2 adjudication).** |
