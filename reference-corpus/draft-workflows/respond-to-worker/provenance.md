...
### `10-validate-target` (folded into `40-apply-and-acknowledge`, N1 adjudication A4)
...
### `20-publish-response` (folded into `40-apply-and-acknowledge`, N1 adjudication A4)
...
### `30-deliver-and-accept` (folded into `40-apply-and-acknowledge`, N1 adjudication A4)
...
### `50-archive-evidence` (folded into `40-apply-and-acknowledge`, N1 adjudication A4)
...
### `60-notify-coordinator` (folded into `40-apply-and-acknowledge`, N1 adjudication A4)
...
### `70-relaunch-if-needed` (folded into `40-apply-and-acknowledge`, N1 adjudication A4)
...
|---|---|---|---|
| `10-validate-target` | none | Swapping the identity/ownership-check implementation leaves the checkpoint — only a respondable-state target with verified identity is accepted — unchanged. | **Demoted** — folded into `40-apply-and-acknowledge` (first). |
| `20-publish-response` | none | Swapping the storage/locking implementation leaves the checkpoint — the response is durably stored before delivery, drain or not — unchanged. | **Demoted** — folded into `40-apply-and-acknowledge` (second). |
| `30-deliver-and-accept` | none | Swapping the readiness-gate implementation leaves the checkpoint — bounded wait, never a fabricated acknowledgement — unchanged. | **Demoted** — folded into `40-apply-and-acknowledge` (third, immediately preceding its judgment). |
| `50-archive-evidence` | none | Swapping the archival implementation leaves the checkpoint — atomic archive, generation fixed at acknowledgement — unchanged. | **Demoted** — folded into `40-apply-and-acknowledge` (fourth, immediately following its judgment). |
| `60-notify-coordinator` | none | Swapping the notification-classification implementation leaves the checkpoint — exactly one durable event kind recorded — unchanged. | **Demoted** — folded into `40-apply-and-acknowledge` (fifth). |
| `70-relaunch-if-needed` | none | Swapping the finalizer-convergence implementation leaves the checkpoint — convergence attempted before refusal, superseded identities preserved — unchanged. | **Demoted** — folded into `40-apply-and-acknowledge` (sixth, last). Its `promote` output disposition is inherited by the merged stage output (see `40-apply-and-acknowledge/output/README.md`). |
| `00-precondition-check`, `40-apply-and-acknowledge` | n/a (extracted as actor-stage, §6.4, already judgment-bearing) | n/a | **Kept** as extracted. |

Stage count: 8 extracted → 2 surviving. No behavior unit was deleted; all units cited under the six folded stages remain cited, now under `40-apply-and-acknowledge`'s "Helper invocations" section (see that stage's `CONTEXT.md`).
