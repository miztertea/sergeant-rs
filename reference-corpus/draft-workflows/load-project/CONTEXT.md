...
| `20-register-or-edit` | actor-stage (§6.4, judgment) | A project definition is written to the Sergeant-owned config path and validated, or the prior definition is restored. |
| `30-sync-repositories` | stage (§6.3, deterministic-machinery candidate — see stage CONTEXT.md) | Every required repo is cloned/refreshed, or the run stops naming the exact failure. |
| `40-report-state` | stage (§6.3, deterministic-machinery candidate — see stage CONTEXT.md) | A read-only per-repo report of clone/branch/cleanliness/ahead-behind and open tracked work. |
