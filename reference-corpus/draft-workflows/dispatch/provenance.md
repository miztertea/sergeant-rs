...
| `60-render-brief` | **yes**, but scope-clarifying only (BU-P5-075..089 authored-not-executed distinction, `--deps` recording-vs-enforcement split) — it does not argue the checkpoint resists implementation swap. | Swapping the rendering/templating implementation leaves the checkpoint — a complete brief durably exists before the worker starts — unchanged. | **Demoted** — folded into `80-monitor`. |
